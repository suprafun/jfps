...
public class ClientServerApp extends JComponent {
    ...
    static TimedPositionInput getInput(long now, Client client) {
        ...
        if (aiGoingLeft && (client1state.x < 50)
                || !aiGoingLeft && (client1state.x > 350)) {
    ...
    static class Client {
        ...
        Client(int index) {
            ...
            interpolatedState = new DelayedInterpolatedState[] {
                new DelayedInterpolatedState(initialState), new DelayedInterpolatedState(initialState)
            };
        }
        ...
        void gameLoop(Server server) {
            while (true) {
                ...
                out.add(new PacketToServer(timedInput,
                        interpolatedState[0].getCurrentTime(), interpolatedState[1].getCurrentTime()));
                if (timedInput.fire) {
    ...
    static class Server {
        ...
        Server(Client[] clients) {
            ...
            animatedStates = new AnimatedState[]{
                        new AnimatedState(clients[0].predictedState.getCurrentState()),
                        new AnimatedState(clients[1].predictedState.getCurrentState()),};
        }
