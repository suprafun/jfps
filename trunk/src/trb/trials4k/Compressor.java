...
/**
 * Original version by Riven (http://indiespot.net/)
 * All I did was convert it to run offline.
 *
 * To use this, you're going to need 7z.exe, BJWFlate.exe, kzip.exe, proguard.jar and proguard.conf in the tools dir.
 *
 * For proguard.conf, I used this:
 *   -target 1.5
 ...
 *   -defaultpackage ''
 *
 * If you can't find BJWFlate.exe, edit the compressors variable.
 */
import java.io.*;
...
public class Compressor {
    ...
    static final File GENERATED_FILE_BASE = new File("tmp");
    static final long EXEC_DELAY = 100;
    ...
    private static String BASE_DIR = new File("").getAbsolutePath() + File.separator;
    ...
    public void manage(String sourcecode, String className) {
        ...
        try {
            ...
            String[] compressors = new String[]{"7z", "kz", "bj"};
                        ...
                        for (String jarType : jarTypes) {
                            ...
                            for (String compressor : compressors) {
                                ...
                                if (compressor.equals("7z")) {
                                    ...
                                    result = bruteforceKzip("compress:kzip (java" + javaVersion + ")", file, toDownload, 0, 4, 64, reruns) != -1;
    ...
    private static void HighLevelSleep(long delay) {
        if (delay > 0) {
            try {
                Thread.sleep(delay);
            } catch (InterruptedException e) {
    ...
    public static void main(String[] args) {
        new Compressor().manage(FileUtil.read(new File("src/" + CLASS_NAME + ".java")), CLASS_NAME);
    }
}
